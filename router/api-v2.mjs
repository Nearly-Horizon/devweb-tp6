...
router.post("/", (req, res) => {
  ...
  res.format({
    "application/json": () => {
      ...
      const secret = nanoid();
      db.prepare(
        "INSERT INTO links (url, origin, secret) VALUES (?, ?, ?)",
    ...
    "text/html": () => {
      ...
      if (!url || typeof url !== "string") {
        return res
          .status(400)
          .render("root", { count, error: "URL manquante" });
      }
      try {
        new URL(url);
      } catch {
        return res.status(400).render("root", { count, error: "URL invalide" });
      }
      ...
      do {
        shortUrl = nanoid();
        if (++attempts > 10)
          return res.status(500).render("root", { count, error: "Erreur" });
      } while (db.prepare("SELECT 1 FROM links WHERE url = ?").get(shortUrl));
...
router.get("/:url", (req, res) => {
  ...
  res.format({
    ...
    "text/html": () => {
      db.prepare("UPDATE links SET visits = visits + 1 WHERE url = ?").run(
        req.params.url,
      );
